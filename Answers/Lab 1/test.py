...
payload = {"app_id":"hajo66","dev_id":"node1","port/channel":randint(0, 100),
...
static
uint8_t
DEV_EUI[] = {0x00, 0x05, 0x4A, 0x1B, 0xB9, 0xE0, 0x1A, 0xF4};
static
uint8_t
APP_EUI[] = {0x70, 0xB3, 0xD5, 0x7E, 0xD0, 0x04, 0x1B, 0x00};
static
uint8_t
APP_KEY[] = {0xCE, 0x23, 0x8E, 0x18, 0x06, 0x1B, 0xA4, 0x10, 0x3C, 0x22, 0x09, 0xD1, 0xCB, 0xE8, 0xF4, 0x20};

// The
port
we
're sending and receiving on
...
sensor and button)
SX1276_LoRaRadio
radio(D11, D12, D13, D10, A0, D2, D3, D4, D5, D8, D9, NC, NC, NC, NC, A4, NC, NC);
Sht31
sht31(I2C_SDA, I2C_SCL);
DigitalOut
led(p5);
InterruptIn
btn(BUTTON1);

// EventQueue is required
to
dispatch
events
around
...
static
void
lora_event_handler(lorawan_event_t
event);
...
while (true) {
led = !led;
printf("LED value is:", led.read());
wait(0.5);
}
// Sending
strings
over
LoRaWAN is not recommended
sprintf((char *)
tx_buffer, "{\"temperature\":%3.1f,\"humidity\":%3.1f,\"LED\":%3.1f}", sht31.readTemperature(), sht31.readHumidity(), led.read());

int
packet_len = strlen((char *)
tx_buffer);
...
main()
{
if (DEV_EUI[0] == 0x0 & & DEV_EUI[1] == 0x0 & & DEV_EUI[2] == 0x0 & & DEV_EUI[3] == 0x0 & & DEV_EUI[4] == 0x0 & &
    DEV_EUI[5] == 0x0 & & DEV_EUI[6] == 0x0 & & DEV_EUI[7] == 0x0)
{
...
trace
output
for this demo, so we can see what the LoRaWAN stack does
mbed_trace_init();
